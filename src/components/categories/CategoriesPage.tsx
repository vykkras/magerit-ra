...
function QuestionnairePanel({ category }: { category: Category }) {
  const { answers, setAnswer, resetCategory } = useQuestionnaireStore();
  ...
  function btn(qid: string, val: Answer) {
    const current = categoryAnswers[qid];
    setAnswer(category.id, qid, current === val ? null : val);
  }
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {answered}/{questions.length} respondidas · {yesCount} Sí · {noCount} No
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                if (confirm('¿Resetear todas las respuestas de este cuestionario?')) resetCategory(category.id);
              }}
              className="text-xs text-slate-400 hover:text-red-500 transition-colors border border-slate-200 hover:border-red-300 px-2.5 py-1 rounded-lg"
            >
              Resetear
            </button>
          </div>
      ...
      {questions.length === 0 && (
        <div className="text-center text-sm text-slate-400 py-12">
          No hay cuestionario definido para esta categoría.
        </div>
      )}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mb-1.5">
                  {q.riskRefs.map(code => {
                    ...
                    const g = t?.group ?? 'E';
                    return (
                      <span key={code} className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${GROUP_COLOR[g]}`}>
                        {code}
                      </span>
                    );
                  })}
